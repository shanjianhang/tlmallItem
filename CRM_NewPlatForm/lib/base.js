...
(function (win, ysp) {
  ...
  utils.extend(ysp.customHelper, {
    ...
    informationEntry: {
      ...
      isSave: false,
      saveData: '',
      enterSplit: function enterSplit(url, str, num) {
        if (!url) {
          throw '未传入必要参数!';
        }
      ...
      sendData: function sendData(obj) {
        if (!((typeof obj === 'undefined' ? 'undefined' : _typeof2(obj)) === 'object') || !(obj instanceof Array)) {
          throw '请传入正确参数类型' + obj;
        }
        // 待客户端补充方法
      ...
      getData: function getData(user, encoded) {
        if (!user && !encoded) {
      ...
      deleteData: function deleteData(user, encoded) {
        if (!user && !encoded) {
          var data = this.basic;
        } else {
          var data = JSON.stringify({
      ...
      backfill: function backfill(doc, titles, obj) {
        if (!doc) {
          throw '未传入正确DOM';
        }
      ...
      filterData: function filterData(doc, titles, id) {
        var tableStats = {
          'thead': true,
          'tbody': true,
          'tr': true
